...
export default function Home() {
  ...
  useEffect(() => {
     ...
     if (lastRoom) setLastRoomId(lastRoom);
  ...
  const handleCreateRoom = () => {
    ...
    socket.emit('room:create', { name, avatar: user?.photoURL || 'default', options: {} }, (response: any) => {
      if (response.success) {
        localStorage.setItem('uno_last_room', response.roomId);
              ...
              <div className="space-y-6">
                ...
                <div className="pt-4 space-y-4">
                  ...
                  {lastRoomId && (
                      <button 
                        onClick={() => {
                            setRoomCode(lastRoomId);
                            handleJoinRoom();
                        }}
                        className="w-full bg-uno-green/20 hover:bg-uno-green text-uno-green hover:text-black font-black py-3 rounded-xl transition-all border border-uno-green/30 uppercase tracking-widest text-xs"
                      >
                        Resume Last Game ({lastRoomId})
                      </button>
                  )}
