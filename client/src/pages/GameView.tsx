...
function TimerBar({ turnEndTime, duration = 15000 }: { turnEndTime: number, duration?: number }) {
    const [timeLeft, setTimeLeft] = useState(100);

    useEffect(() => {
        const interval = setInterval(() => {
            const now = Date.now();
            const remaining = Math.max(0, turnEndTime - now);
            const percentage = (remaining / duration) * 100;
            setTimeLeft(percentage);
        }, 100);
        return () => clearInterval(interval);
    }, [turnEndTime, duration]);

    return (
        <div className="w-full h-1.5 bg-white/10 rounded-full mt-2 overflow-hidden">
            <motion.div 
                className={`h-full ${timeLeft < 25 ? 'bg-red-500 shadow-[0_0_10px_rgba(239,68,68,0.5)]' : 'bg-uno-accent shadow-[0_0_10px_rgba(157,0,255,0.5)]'}`}
        ...
        >
            ...
            <div className="flex-1 flex items-center justify-center relative z-10">
                ...
                <div className="absolute bottom-4 md:bottom-auto md:right-12 bg-uno-dark/80 backdrop-blur-md p-4 rounded-3xl border border-white/10 shadow-2xl min-w-[160px]">
                    ...
                    {gameState.turnEndTime && <TimerBar turnEndTime={gameState.turnEndTime} duration={gameState.isBot ? 3000 : 15000} />}
            ...
            {/* Hand Area */}
            <div className="bg-gradient-to-t from-black/80 to-transparent pt-12 pb-6 flex flex-col items-center">
                <div className="flex justify-center -space-x-12 md:-space-x-16 max-w-full overflow-x-auto py-8 px-12 scrollbar-hide">
                    <AnimatePresence>
                        {hand.map((card, index) => (
                            <motion.div
                                key={card.id}
                                initial={{ y: 200, opacity: 0 }}
                                animate={{ y: 0, opacity: 1, rotate: (index - hand.length/2) * 2 }}
                                whileHover={{ y: -40, scale: 1.1, zIndex: 100, rotate: 0 }}
                                className="relative origin-bottom"
                                style={{ zIndex: index }}
                            >
                                <Card card={card} onClick={handlePlayCard} disabled={!isMyTurn} className={`transition-all duration-300 ${!isMyTurn ? 'brightness-50 grayscale-[0.2]' : ''}`} />
                            </motion.div>
                        ))}
                    </AnimatePresence>
                </div>
            ...
            <AnimatePresence>
                ...
                {showTargetSelector && <TargetSelector players={gameState.players} currentPlayerId={myPlayer?.id} onSelect={handleTargetSelected} onCancel={() => setShowTargetSelector(false)} />}
                {showWinnerModal && (
                    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="fixed inset-0 z-[100] flex items-center justify-center bg-black/90 backdrop-blur-xl">
                        <div className="text-center">
                            ...
                            <p className="text-2xl text-uno-accent font-bold mb-8 uppercase tracking-widest">{showWinnerModal.name} WINS</p>
                            <button onClick={() => navigate('/')} className="bg-uno-accent hover:bg-purple-500 text-white font-black px-12 py-4 rounded-2xl shadow-2xl transition-all active:scale-95">CONTINUE</button>
                        </div>
                    </motion.div>
                )}
            </AnimatePresence>
