...
function Card({ card, onClick, disabled = false, style, className = '' }) {
   ...
   return (
      ...
      >
          ...
          >
              {/* Inner Halo / Glow - Simplified */}
              <div className={`absolute inset-0 opacity-30 ${haloStyle}`}></div>
              
              {/* Corner Symbols */}
              <div className={`absolute top-1.5 left-2 font-black text-[10px] md:text-xs ${textColor}`}>
                  {text}
              </div>
              <div className={`absolute bottom-1.5 right-2 font-black text-[10px] md:text-xs transform rotate-180 ${textColor}`}>
                  {text}
              </div>

              {/* Large Center Symbol */}
              <div className="flex-1 h-full flex justify-center items-center relative z-10 p-2 text-center">
                  <span className={`text-xl md:text-3xl font-black italic tracking-tighter drop-shadow-[0_0_5px_rgba(0,0,0,0.5)] ${textColor}`}>
                      {text}
                  </span>
              </div>
              
              {/* Premium Glossy Layer - Simplified */}
